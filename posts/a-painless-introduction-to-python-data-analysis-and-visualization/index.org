...
Slices are of the form ~[start:stop:step]~. This takes the elements starting at ~start~ and going up to
~stop~ (but not including it!), taking every ~step~ element. If there are only two arguments, it's
instead ~[start:stop]~ and step is assumed to be 1. Additionally, slices of the form ~[:stop]~ and
~[start:]~ are perfectly fine: the former assumes ~start~ is ~0~ and the latter assumes that ~stop~ is the
length of the list. (This means ~[:stop]~, because it doesn't include the value at ~stop~ but starts at
~0~, is the first ~stop~ elements. This is very useful to keep in mind.) The last thing to know is that
you can use negative indices: in this case, it'll count backwards from the end of the list. ~-1~ means
"the last element", and ~-3~ means "the third-to-last element". (You can think of these as equivalent
to ~len(list) - 1~ and ~len(list) - 3~.)
...
Option number four is tempting, but we're going to take the first options: the problematic rows have
